...
// (/picks, recaps) applies the same per-market isotonic calibration the Python
...
// Test/refresh hook: clear the memoized maps so the next call re-reads disk.
 ...
 * Falls back to the raw probability when no map exists for the market, so an
 * uncalibrated market is never worse than the model's own estimate.
 */
